...
- 全新的设置 app.你可以设置 argon 主题的登录页面的模糊和透明度，并管理背景图片与视频。[建议使用 Chrome][点击下载](https://github.com/jerrykuku/luci-app-argone-config/releases/download/v0.8-beta/luci-app-argone-config_0.8-beta_all.ipk)
- 当编译固件时，将自动设置为默认主题。
- 修改文件结构，以适应 luci-app-argone-config，旧的开启暗色模式方法将不再适用，请搭配 luci-app-argone-config 使用。
...
- 背景文件策略调整为，同时接受 jpg png gif mp4, 自行上传文件至 /www/luci-static/argone/background 图片和视频同时随机。
